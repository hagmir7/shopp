<div class="space-y-4" id="buy-now">
    ...
    <div class="mb-6">
        <div class="flex items-center w-full input-primary py-2">
            <button type="button" wire:click="decrementQuantity"
                class="px-4 py-1 text-gray-600 hover:bg-gray-100 text-xl font-bold">
                -
            </button>
            <input type="number" min="1" wire:model="quantity" value="{{ $quantity }}"
    ...
    <div class="grid md:grid-cols-2 gap-4">
        <div>
            <input type="text" wire:model.live.debounce.500ms="full_name" class="input-primary" placeholder="{{ __('Full name') }}">
            @error('full_name') <span class="text-red-600 mt-2">{{ $message }}</span> @enderror
        </div>
        <div>
            <input type="tel" wire:model.live.debounce.500ms="phone" class="input-primary"
    ...
    <div>
        <input type="text" wire:model.live.debounce.500ms="city" class="input-primary mb-3"
            placeholder="{{ __('City') }}">
        @error('city') <span class="text-red-600 mt-2">{{ $message }}</span> @enderror

        <input type="text" wire:model.live.debounce.500ms="address" class="input-primary mb-3"
            placeholder="{{ __('Address') }}">
        @error('address') <span class="text-red-600 mt-2">{{ $message }}</span> @enderror
    </div>

    <!-- SUBMIT -->
    <button type="button" wire:click.prevent="save" wire:target="save() wire:loading.attr="disabled"  class="btn btn-primary w-full rounded-md">
