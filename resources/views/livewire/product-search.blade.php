<div class="flex-1 max-w-xl mx-4 hidden lg:block">
    <div class="relative">
        ...
        <p class="absolute left-3 top-1/2 -translate-y-1/2">
            ...
            <svg wire:loading role="status" aria-hidden="true" class="w-6 h-6 [animation:spin_0.5s_linear_infinite] text-gray-600 fill-blue-600"
        ...
        <!-- Search Input -->
        <input type="text" wire:model.live.debounce.300ms="query" placeholder="{{ __('Search for products') }}"
            class="input-primary w-full pl-10 pr-4 py-2 rounded-full">
        ...
        <div
            ...
            <a href="{{ route('product.show', $result->slug) }}"
                ...
                <img src="{{ $result->images->first()?->path ? Storage::url($result->images->first()->path) : 'https://via.placeholder.com/64' }}"
                    alt="{{ $result->name }}" class="w-16 h-16 object-cover rounded">
                <div class="ml-4">
