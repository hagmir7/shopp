<div class="flex items-center justify-center lg:py-0 w-full">
    <div class="w-full bg-white rounded-lg max-w-2xl shadow">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            <form class="space-y-4 md:space-y-6" wire:submit.prevent='register()'>
                <div class="flex gap-2">
                    <div class="w-full">
                        <label for="name" class="block mb-1 text-sm font-medium text-gray-900">{{ __("First name") }}</label>
                        <input type="text" wire:model='first_name' class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" placeholder="{{ __("First name") }}">
                        @error('first_name') <span class="text-red-700">{{ $message }}</span> @enderror
                    </div>
                    <div  class="w-full">
                        <label for="name" class="block mb-1 text-sm font-medium text-gray-900">{{ __("Last name") }}</label>
                        <input type="text" wire:model='last_name' class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" placeholder="{{ __("Last name") }}">
                        @error('last_name') <span class="text-red-700">{{ $message }}</span> @enderror
                    </div>
                </div>
                <div class="flex gap-2">
                    <div class="w-full">
                            <label for="phone" class="block mb-1 text-sm font-medium text-gray-900">{{ __("Phone") }}</label>
                            <input type="tel" wire:model='phone' class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-red-600 focus:border-red-600 block w-full p-2.5" placeholder="{{ __('Your phone') }}">
